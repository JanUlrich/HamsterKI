...
public class DebuggerTools implements Observer, ChangeListener {
	...
	public DebuggerTools(DebuggerModel model, DebuggerController controller) {
		this.model = model;
		model.addObserver(this);

		JMenu menu = controller.getWorkbench().getView()
				.findMenu("editor", "debugger");
		...
		JToolBar toolBar = controller.getWorkbench().getView()
				.findToolBar("editor");
		...
		JToolBar simulationBar = controller.getWorkbench().getView()
				.findToolBar("simulation");
		simulationBar.add(Box.createRigidArea(new Dimension(11, 11)));
		simulationBar.add(Utils.createButton(startAction));
		simulationBar.add(Box.createRigidArea(new Dimension(2, 2)));
		simulationBar.add(Utils.createButton(pauseAction));
		simulationBar.add(Box.createRigidArea(new Dimension(2, 2)));
		simulationBar.add(Utils.createButton(stopAction));
		...
		// chris
		JToolBar simulationBar3D = controller.getWorkbench().getView()
				.findToolBar("3dsimulation");
	...
	public void updateButtonStates() {
		...
		// Martin + Python + Ruby
		if (activeFile != null
				&& (activeFile.getType() == HamsterFile.SCHEMEPROGRAM
						|| activeFile.getType() == HamsterFile.PYTHONPROGRAM || activeFile
						.getType() == HamsterFile.RUBYPROGRAM)) {
			enableAction.setEnabled(false);
			stopAction
					.setEnabled(model.getState() != DebuggerModel.NOT_RUNNING);
			pauseAction.setEnabled(false);
			stepAction.setEnabled(false);
			stepOverAction.setEnabled(false);
			startAction
					.setEnabled(activeFile != null
							&& (model.getState() != DebuggerModel.RUNNING && !(activeFile
									.getType() == HamsterFile.HAMSTERCLASS && model
									.getState() == DebuggerModel.NOT_RUNNING)));
			/*
			 * diboxy if (de.hamster.workbench.Workbench.workbench != null) {
			 * de.hamster.workbench.Workbench.getWorkbench().getSimulation()
			...
			de.hamster.workbench.Workbench
					...
					.getResetAction()
					.setEnabled(
							de.hamster.workbench.Workbench.getWorkbench()
									.getSimulation().getSimulationModel().savedTerrain != null);
			// Prolog
		} else if (activeFile != null
				&& activeFile.getType() == HamsterFile.PROLOGPROGRAM) {
			...
			de.hamster.workbench.Workbench
					...
					.setEnabled(
							de.hamster.workbench.Workbench.getWorkbench()
									.getSimulation().getSimulationModel().savedTerrain != null
									&& (model.getState() != DebuggerModel.RUNNING && model
											.getState() != DebuggerModel.PAUSED));
		} else {
			startAction
					.setEnabled(activeFile != null
							&& (model.getState() != DebuggerModel.RUNNING && !(activeFile
			...
			if (activeFile != null
				...
				stepAction.setEnabled(model.isEnabled()
						&& (model.getState() == DebuggerModel.PAUSED || (model
								.getState() == DebuggerModel.NOT_RUNNING)
								&& startAction.isEnabled()));
				stepOverAction.setEnabled(model.isEnabled()
						...
						.setEnabled(
								de.hamster.workbench.Workbench.getWorkbench()
										...
										&& (model.getState() != DebuggerModel.RUNNING
												&& model.getState() != DebuggerModel.PAUSED && !(activeFile
												.getType() == HamsterFile.HAMSTERCLASS && model
												.getState() == DebuggerModel.NOT_RUNNING)));
			}
		}
