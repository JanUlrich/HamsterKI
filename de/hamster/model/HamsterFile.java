...
public class HamsterFile implements Comparable {
	...
	public static HamsterFile getHamsterFile(String absolute) {
		if (allFiles.get(absolute) == null) {
			...
			if (HamsterFile.isScratchProgram(f)) {
				file = new ScratchHamsterFile(new File(absolute));
			} else if (HamsterFile.isFSMProgram(f)) {
				file = new FsmHamsterFile(new File(absolute));
			} else if (HamsterFile.isFlowchartProgram(f)) {
				file = new FlowchartHamsterFile(new File(absolute));
			} else {
	...
	public static HamsterFile createHamsterFile(String absolute, char type) {
		if (allFiles.get(absolute) == null) {
			File f = new File(absolute);
			HamsterFile file = null;
			if (type == HamsterFile.SCRATCHPROGRAM) {
				file = new ScratchHamsterFile(new File(absolute));
	...
	static void recursiveCopy(File file, File toDir) {
		...
		if (file.isDirectory()) {
			...
			if (oldHamsterFile.getType() == HamsterFile.SCRATCHPROGRAM) {
				((ScratchHamsterFile) newHamsterFile)
						.setProgram(new ScratchProgram(
								((ScratchHamsterFile) oldHamsterFile)
										.getProgram()));
			} else if (oldHamsterFile.getType() == HamsterFile.FSM) {
				((FsmHamsterFile) newHamsterFile).setProgram(new FsmProgram(
						((FsmHamsterFile) oldHamsterFile).getProgram(),
						(FsmHamsterFile) newHamsterFile));
			} else if (oldHamsterFile.getType() == HamsterFile.FLOWCHART) {
				((FlowchartHamsterFile) newHamsterFile)
						.setProgram(new FlowchartProgram(
								((FlowchartHamsterFile) oldHamsterFile)
										.getProgram(),
								(FlowchartHamsterFile) newHamsterFile));
			}
	...
	public String load() {
		...
		if (isTerrain()) { // dibo 260110
			type = TERRITORIUM;
		} else if (isScratchProgram(s)) { // Scratch
			type = SCRATCHPROGRAM;
		} else if (isFSMProgram(s)) { // FSM
			type = FSM;
		} else if (isFlowchartProgram(s)) { // Flowchart
			type = FLOWCHART;
		} else if (isProgram()) {
			type = IMPERATIVE;
			int pos = s.indexOf("*/");
	...
	static boolean isScratchProgram(String str) {
		try {
			return str.trim().contains(
					"<" + ScratchHamsterFile.SCRATCH_TAG + ">");
		} catch (Throwable th) {
			return false;
		}
	}

	static boolean isFSMProgram(String str) {
		try {
			return str.trim().contains("<" + FsmHamsterFile.FSM_TAG + ">");
		} catch (Throwable th) {
			return false;
		}
	}

	static boolean isFlowchartProgram(String str) {
		try {
			return str.trim().contains(
					"<" + FlowchartHamsterFile.FLOWCHART_TAG + ">");
		} catch (Throwable th) {
			return false;
		}
	}

	static boolean isScratchProgram(File f) {
		...
		String s = new String(buffer);
		return isScratchProgram(s);
	}

	static boolean isFSMProgram(File f) {
		if (!f.exists() || f.isDirectory()) {
			return false;
		}
		byte[] buffer = null;
		...
		String s = new String(buffer);
		return isFSMProgram(s);
	}

	static boolean isFlowchartProgram(File f) {
		if (!f.exists() || f.isDirectory()) {
			return false;
		}
		byte[] buffer = null;
	...
	public void save(String text) {
		...
		} else if (type == SCRATCHPROGRAM || type == FSM || type == FLOWCHART) { // Scratch, FSM
			// no prefix
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					Utils.getResource("editor.saveerror"), "Fehler",
