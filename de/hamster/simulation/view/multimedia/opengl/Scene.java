...
/**
 * Diese Klasse enthält die Daten für die Darstellung 
 * der Hamsterwelt und die Positionen.
 * Sie enthält die Daten in aufbereiteter Form für die
 * Darstellung in 3D. (Interpolierte Hamsterposition, etc.)
 * 
 * Diese Klasse enthält die technischen Aspekte der Szene 
 * (Kamera etc.), inhaltliche sind in der Hamsterworld.
 */
...
public class Scene implements Observer {
	...
	public Scene(SimulationModel simmodel, DebuggerModel debmodel, GLDrawable gld) {
		...
		// hier haben wir einen gl-kontext, können also die texturen laden lassen:
		this.materialController.doTextureRefresh(gld);
		
	}

	public void setPerspectiveCamPosition(float neigung, float rotation, float dist, float transX, float transY) {
		...
		// diese werte errechnen sich aus der kartengröße und zentrieren die karte:
	 ...
	 * 
	 * Zeichnet die Szene.
	 * Erhält Systemzeit als Parameter um Animationen zu interpolieren. 
	 */
	public void draw(GLDrawable gld, long time) {
	...
	public void update(Observable o, Object arg) {
		...
		if (positions!=null) positions.addStage();		
	}
