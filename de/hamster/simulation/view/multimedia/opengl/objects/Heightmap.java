...
public class Heightmap {
	...
	public void drawGrid(GLDrawable gld) {
		...
		gl.glMaterialfv( GL.GL_FRONT, GL.GL_AMBIENT,   this.c1.getData()  );
		gl.glMaterialfv( GL.GL_FRONT, GL.GL_DIFFUSE,   this.c2.getData()  );
		...
		for (xpos=border; xpos<(this.mapSizeX-1) - border +1; xpos+=1) {			
			...
			for (ypos=border; ypos<(this.mapSizeY-1) - border +1; ypos+=1) {
				...
				if (ypos<(this.mapSizeY-1) - border) {
					gl.glVertex3fv(ol.getData());
					gl.glVertex3fv(ul.getData());
				}
				
				if (xpos<(this.mapSizeX-1) - border ) {
					gl.glVertex3fv(ul.getData());
					gl.glVertex3fv(ur.getData());
				}
