...
public class SoundManager {
	...
	public SoundManager () {
		for (int i=0; i<this.MAX_SOUNDS; i++) sounds[i] = null;
	...
	public void closeAll() {
		for (int i=0; i<this.MAX_SOUNDS; i++)
			if (sounds[i] != null) this.sounds[i].close();		
	}
	...
	public void setSpeed(int speed) {
		...
		for (int i=0; i<this.MAX_SOUNDS; i++) {
			...
			if (sounds[i] != null) { 
				this.sounds[i].stop();
				SoundClip neu = new SoundClip(this.sounds[i].filename, ((SoundClip)this.sounds[i]).type, speed);				
				this.sounds[i] = neu;
			}
		}
