...
public class WorkbenchView implements Observer, WindowFocusListener {
	...
	public class ShowManualAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				desktop
						.browse(new URI(
								"http://www-is.informatik.uni-oldenburg.de/~dibo/hamster/download/v28/handbuch-html/handbuch.htm"));
	...
	public class ShowAPIAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				desktop
						.browse(new URI(
								"http://www-is.informatik.uni-oldenburg.de/~dibo/hamster/band2/API/index.html?package-summary.html"));

			} catch (Exception exc) {
				new WebBrowser(
						Utils.getResources().getString("workbench.api.text"),
						"http://www-is.informatik.uni-oldenburg.de/~dibo/hamster/band2/API/index.html?package-summary.html");
			}
		}
	}
	...
	public class ShowImpAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				new WebBrowser(Utils.getResources().getString(
						"workbench.imp.text"),
						"http://books.google.com/books?id=_pQ-9QTpcZMC&hl=de");
			}
		}
	...
	public void createEditorFrame() {
		...
		debugPanel.add(new JScrollPane(workbench.getDebugger()
				.getStackFrameTable()));
		debugPanel.add(new JScrollPane(workbench.getDebugger()
				.getVariableViewer()));
		...
		workbench.getComiler().getErrorPanel().setBorder(
		...
		workbench.getEditor().getTabbedTextArea().setBorder(
				BorderFactory.createEmptyBorder(0, 0, 1, 1));
		mainPanel.add(BorderLayout.CENTER, workbench.getEditor()
				.getTabbedTextArea());

		// Anpassen des Dateibaums
		JScrollPane fileTree = new JScrollPane(workbench.getEditor()
				.getFileTree());
		fileTree.setBorder(BorderFactory.createCompoundBorder(BorderFactory
				.createEmptyBorder(0, 0, 0, 5), fileTree.getBorder()));
		fileTree.setPreferredSize(new Dimension(150, 150));

		// Hinzufuegen von Dateibaum und Main-Panel
		// in SplitPane (dibo)
		JSplitPane sp = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, fileTree,
				mainPanel);
	...
	public JToolBar findToolBar(String id) {
		...
		if (toolBar == null) {
			toolBar = new JToolBar(resources.getString("toolbar." + id
					+ ".text"));
	...
	public JMenu findMenu(String bar, String key) {
		...
		if (menu == null) {
			menu = new JMenu(resources.getString("menu." + bar + "." + key
					+ ".text"));
	...
	public void update(Observable arg0, Object arg) {
		if (arg == CompilerModel.COMPILER_ERRORS) {
			if (model.getCompilerModel().getCompilerErrors().size() == 0) {
				JOptionPane.showMessageDialog(editor, Utils
						.getResource("workbench.kompilierung"));
				// Compiler-Fehler ausblenden
				mainPanel.remove(workbench.getComiler().getErrorPanel());
			} else {
				// Compiler-Fehler einblenden
				mainPanel.add(BorderLayout.SOUTH, workbench.getComiler()
						.getErrorPanel());
			}
		...
		/* lego */
			...
			if (model.getLegoModel().getState() == LegoModel.SUCCESS) {
				JOptionPane.showMessageDialog(editor, Utils
						.getResource("workbench.lego.success"));
			} else if (model.getLegoModel().getState() == LegoModel.FAILURE) {
				JOptionPane.showMessageDialog(editor, Utils
						.getResource("workbench.lego.failure"));
			}
		}
